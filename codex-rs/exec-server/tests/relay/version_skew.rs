...
#[derive(Clone, Copy)]
enum BinaryGeneration {
    Current,
    Released,
}

impl BinaryGeneration {
    fn configure_compatibility_auth(self, command: &mut Command) {
        match self {
            Self::Current => {
                command.env_remove("CODEX_API_KEY");
            }
            Self::Released => {
                command.env("CODEX_API_KEY", REGISTRY_TOKEN);
            }
        }
    }
}
...
async fn current_app_server_runs_commands_on_released_exec_server_over_noise() -> Result<()> {
    ...
    assert_noise_version_skew(
        &current,
        BinaryGeneration::Current,
        &released,
        BinaryGeneration::Released,
    )
    .await
}
...
async fn assert_noise_version_skew(
    app_binary: &Path,
    app_generation: BinaryGeneration,
    executor_binary: &Path,
    executor_generation: BinaryGeneration,
) -> Result<()> {
    let codex_home = TempDir::new()?;
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture::new(REGISTRY_TOKEN)
...
"#
    ...
    let mut executor_command = Command::new(executor_binary);
    executor_command
        .args([
            "exec-server",
            "--remote",
        ...
        .kill_on_drop(true);
    executor_generation.configure_compatibility_auth(&mut executor_command);
    let mut executor = executor_command
        .spawn()
    ...
    let mut app_server_command = Command::new(app_binary);
    app_server_command
        .arg("app-server")
        .current_dir(codex_home.path())
        .env("CODEX_HOME", codex_home.path())
        ...
        .stderr(Stdio::inherit())
        .kill_on_drop(true);
    app_generation.configure_compatibility_auth(&mut app_server_command);
    let mut app_server = app_server_command
        .spawn()
