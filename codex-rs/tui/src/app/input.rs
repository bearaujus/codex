...
impl App {
    ...
    pub(super) async fn handle_key_event(
        ...
        {
            // Undo Send advertises Esc as an immediate, local cancellation.
            // It must win over the app-level Esc-Esc backtrack state machine
            // while a staged submission is visible.
            if self.chat_widget.has_staged_submission()
                && self.chat_widget.no_modal_or_popup_active()
            {
                self.reset_backtrack_state();
    ...
    pub(super) fn should_handle_backtrack_esc(&self, key_event: KeyEvent) -> bool {
        !self.chat_widget.side_conversation_active()
            && !self.chat_widget.has_staged_submission()
