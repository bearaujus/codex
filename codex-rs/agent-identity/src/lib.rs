...
-----END PRIVATE KEY-----"#,
    ...
    #[test]
    fn registration_http_error_includes_response_body() {
        let error = AgentIdentityRegistrationHttpError::new(
            "agent task registration",
        ...
        assert_eq!(
            error.to_string(),
            "agent task registration failed with status 502 Bad Gateway: upstream unavailable"
        );
    }
