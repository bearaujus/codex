...
/// Open a writable SQLite pool with one connection and full synchronous durability.
///
/// Use this for small credential or metadata databases whose writes must remain serialized and
/// durable across abrupt process termination.
pub async fn open_durable_single_connection_sqlite_pool(path: &Path) -> Result<SqlitePool, Error> {
...
/// Open a direct connection to an existing SQLite database.
///
/// Prefer a pool for application workloads. This entrypoint exists for serialized maintenance and
/// focused tests that need connection-scoped behavior.
pub async fn open_existing_sqlite_connection(path: &Path) -> Result<SqliteConnection, Error> {
    let options = SqliteConnectOptions::new()
        .filename(path)
