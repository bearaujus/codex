...
mod tests {
    ...
    #[tokio::test]
    async fn test_handle_token_count_event_without_rate_limits_preserves_cached_limits()
    -> Result<()> {
        let (tx, mut rx) = mpsc::channel(CHANNEL_CAPACITY);
        let outgoing = Arc::new(OutgoingMessageSender::new(
            tx,
        ...
        let token_usage = recv_broadcast_notification(&mut rx).await?;
        assert!(matches!(
            token_usage,
        ...
        assert!(
            rx.try_recv().is_err(),
            "missing rate-limit data must not erase the client's cached snapshot"
        );
        Ok(())
    }
