...
async fn git_attribution_follows_authenticated_workspace_policy() -> Result<()> {
    ...
    write_chatgpt_auth_to_pool(
        codex_home.path(),
        ChatGptAuthFixture::new("disabled-token")
            .account_id("workspace-disabled")
            .plan_type("enterprise"),
        AuthCredentialsStoreMode::File,
    )
    .await?;
    mark_pool_auth_failed(codex_home.path(), "workspace-enabled").await?;
    ...
    write_chatgpt_auth_to_pool(
        codex_home.path(),
        ChatGptAuthFixture::new("enabled-token")
            .account_id("workspace-enabled")
            .plan_type("enterprise"),
        AuthCredentialsStoreMode::File,
    )
    .await?;
    mark_pool_auth_failed(codex_home.path(), "workspace-disabled").await?;
