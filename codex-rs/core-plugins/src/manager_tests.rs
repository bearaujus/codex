...
fn plugins_manager_tracks_auth_mode() {
    ...
    assert!(!manager.set_auth_mode(Some(AuthMode::Chatgpt)));
    assert!(manager.set_auth_mode(Some(AuthMode::Headers)));
...
async fn plugin_auth_projection_reprojects_cached_plugins_when_auth_changes() {
    ...
    assert!(manager.set_auth_mode(/*auth_mode*/ None));
    let unauthenticated_outcome = manager.plugins_for_config(&config).await;

    assert_eq!(
        sorted_effective_mcp_server_names(&unauthenticated_outcome),
        vec!["docs".to_string(), "sample".to_string()]
    );
    assert!(unauthenticated_outcome.effective_apps().is_empty());
    assert_eq!(
        unauthenticated_outcome.capability_summaries(),
        &[
            PluginCapabilitySummary {
                config_name: "docs@test".to_string(),
                display_name: "docs".to_string(),
...
"#,
    ...
    for auth_mode in [AuthMode::Headers, AuthMode::AgentIdentity] {
...
"#,
    ...
    let unauthenticated_outcome = PluginsManager::new_with_options(
        tmp.path().to_path_buf(),
        Some(Product::Codex),
        /*auth_mode*/ None,
    )
    ...
    .unwrap();
    assert_eq!(
        unauthenticated_outcome.plugin.mcp_server_names,
        vec!["other-mcp".to_string(), "sample-mcp".to_string()]
    );
    assert_eq!(
        unauthenticated_outcome.plugin.apps,
        vec![AppConnectorId("connector_sample".to_string())]
    );
}
