...
async fn run_compact_task_inner(
    ...
    let pre_compact_outcome =
        run_pre_compact_hooks(&sess, &turn_context, compaction_metadata.trigger()).await;
    ...
    if result.is_ok() {
        let post_compact_outcome =
            run_post_compact_hooks(&sess, &turn_context, compaction_metadata.trigger()).await;
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(e) if matches!(e.details(), CodexErrorDetails::SessionBudgetExceeded) => {
                emit_standalone_compaction_error(
                    sess.as_ref(),
                    turn_context.as_ref(),
                    compaction_metadata.trigger(),
                    &e,
            ...
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = stream_reconnect_delay(retries);
...
async fn emit_standalone_compaction_error(
    sess: &Session,
    turn_context: &TurnContext,
    trigger: CompactionTrigger,
    error: &CodexErr,
) {
    // The regular turn owns account-pool retry decisions. Suppress only those
    // retry candidates so a successful failover stays invisible; all other
    // inline failures retain the established compaction error lifecycle.
    if !matches!(trigger, CompactionTrigger::Manual) && is_account_pool_failover_error(error) {
        return;
    }
    sess.track_turn_codex_error(turn_context, error);
    sess.send_event(
        turn_context,
        EventMsg::Error(error.to_error_event(Some("Error running local compact task".to_string()))),
    )
    .await;
}

pub(crate) fn is_account_pool_failover_error(error: &CodexErr) -> bool {
    matches!(
        error.details(),
        CodexErrorDetails::UsageLimitReached(_) | CodexErrorDetails::RefreshTokenFailed(_)
    )
}
    ...
    loop {
        ...
        match event {
            ...
            Ok(ResponseEvent::RateLimits(snapshot)) => {
                if let Some(account_id) = client_session.last_request_pool_account_id() {
                    sess.services
                        .auth_manager
