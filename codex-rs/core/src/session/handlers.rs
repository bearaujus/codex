...
pub(super) async fn user_input_or_turn_inner(
    ...
    if !prepare_account_pool_for_task(sess, current_context.as_ref(), &sub_id).await {
        return Err(CodexErr::InvalidRequest(
            "account pool could not prepare an eligible account".to_string(),
        ));
    }
...
/// Re-evaluates shared account-pool state immediately before a model-backed
/// task starts.
///
/// This applies service-side enable, cooldown, and credential changes to
/// manual compaction as well as normal user turns. If no eligible account
/// remains, the task stops before it can send a stale cached credential.
async fn prepare_account_pool_for_task(
    sess: &Arc<Session>,
...
pub async fn compact(sess: &Arc<Session>, sub_id: String) {
    let turn_context = sess.new_default_turn_with_sub_id(sub_id).await;
    if !prepare_account_pool_for_task(sess, turn_context.as_ref(), &turn_context.sub_id).await {
        return;
    }
