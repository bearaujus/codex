...
impl std::fmt::Debug for AgentIdentityStorage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Jwt(jwt) => f
                .debug_struct("AgentIdentityJwt")
                .field("jwt_set", &!jwt.is_empty())
                .finish(),
            Self::Record(record) => f.debug_tuple("AgentIdentityRecord").field(record).finish(),
        }
    }
}
