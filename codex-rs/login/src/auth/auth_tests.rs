...
#[test]
fn auth_debug_redacts_token_and_agent_identity_material() {
    const SENTINEL: &str = "secret-auth-material";
    let auth = AuthDotJson {
        ...
        last_refresh: None,
        agent_identity: Some(AgentIdentityStorage::Jwt(format!("{SENTINEL}-agent"))),
    };

    let rendered = format!("{auth:?}");
    assert!(
        !rendered.contains(SENTINEL),
        "AuthDotJson Debug leaked credentials: {rendered}"
    );
    assert!(rendered.contains("access_token_set: true"));
    assert!(rendered.contains("jwt_set: true"));
}
...
#[tokio::test]
async fn refresh_response_with_omitted_token_fields_preserves_existing_values() {
    let codex_home = tempdir().unwrap();
    ...
    let before_tokens = before.tokens.expect("tokens should exist");

    let updated = super::persist_tokens(
        &storage, /*id_token*/ None, /*access_token*/ None, /*refresh_token*/ None,
    )
    .expect("optional refresh fields should preserve stored credentials");

    let tokens = updated.tokens.expect("tokens should exist");
    assert_eq!(tokens.id_token.raw_jwt, original_id_token);
    assert_eq!(tokens.access_token, before_tokens.access_token);
    assert_eq!(tokens.refresh_token, before_tokens.refresh_token);
    assert!(updated.last_refresh.is_some());
}

#[tokio::test]
#[serial(codex_auth_env)]
async fn malformed_successful_refresh_response_is_terminal() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/oauth/token"))
        .respond_with(ResponseTemplate::new(200).set_body_string("not token JSON"))
        .expect(1)
        .mount(&server)
        .await;
    let endpoint = format!("{}/oauth/token", server.uri());
    let _endpoint_guard = EnvVarGuard::set(REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR, &endpoint);
    ...
    let err = match request_chatgpt_token_refresh("refresh-potentially-spent".to_string(), &client)
        .await
    {
        Err(err) => err,
        Ok(_) => panic!("a malformed successful response must not be retried"),
    };

    assert!(matches!(
        err,
        RefreshTokenError::Permanent(RefreshTokenFailedError {
            reason: RefreshTokenFailedReason::Other,
            ..
        })
    ));
    server.verify().await;
...
#[tokio::test]
#[serial(codex_auth_env)]
async fn oversized_successful_refresh_response_is_terminal() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/oauth/token"))
        .respond_with(
            ResponseTemplate::new(200).set_body_string("x".repeat(MAX_AUTH_SUCCESS_BODY_BYTES + 1)),
        )
        .expect(1)
        .mount(&server)
        .await;
    ...
    {
        Err(err) => err,
        Ok(_) => panic!("an oversized successful response must not be retried"),
    };
...
#[tokio::test]
#[serial(codex_auth_env)]
async fn failed_refresh_does_not_expose_response_body() {
    ...
    let err = match request_chatgpt_token_refresh(SENTINEL.to_string(), &client).await {
        Err(err) => err,
        Ok(_) => panic!("a failed refresh must return an error"),
    };
    let rendered = err.to_string();

    assert!(
        !rendered.contains(SENTINEL),
        "refresh error leaked response body: {rendered}"
    );
    assert!(
        rendered.contains("error_code=server_error"),
        "refresh error lost safe diagnostic code: {rendered}"
    );
    server.verify().await;
}
...
async fn startup_preserves_top_level_auth_when_pool_copy_does_not_match() {
    ...
    let mut conn = codex_state::open_existing_sqlite_connection(&crate::account_pool_db_path(
        codex_home.path(),
    ))
    .await
...
async fn cli_rate_limit_reads_use_cached_pool_data_without_usage_refresh() {
    ...
    pool.record_validated_rate_limits(WORKSPACE_ID_ALLOWED, std::slice::from_ref(&cached_snapshot))
        .await
    ...
    let account = manager
        ...
        .into_iter()
        .find(|account| account.account_id == WORKSPACE_ID_ALLOWED)
        .expect("active account should remain in the pool");
    assert_eq!(
        account.cooldown_until, None,
        "a successful turn must not be reclassified by a stale cached limit"
    );
...
#[tokio::test]
#[serial(codex_auth_env)]
async fn pool_logout_revokes_and_invalidates_every_service_owned_credential() {
    let codex_home = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    write_auth_file(
        AuthFileParams {
    ...
    let refresh_token = pool_auth
        ...
        .clone();
    manager
        .activate_pool_managed_auth(pool_auth)
        .await
        .expect("pool auth should activate");
    ...
    Mock::given(method("POST"))
        ...
        .mount(&revoke_server)
        .await;
    let _refresh_endpoint_guard = EnvVarGuard::set(
        REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR,
        &format!("{}/oauth/token", revoke_server.uri()),
    );

    manager
        .logout_with_revoke()
        .await
        .expect("pool logout should succeed");
    ...
    assert_eq!(
        accounts[0].auth_status,
        ChatgptAccountPoolAuthStatus::Invalid
    );
    let retained = pool
        .read_account_tokens(WORKSPACE_ID_ALLOWED)
        .await
        .expect("retained credential should load")
        .expect("retained credential should exist");
    assert_eq!(
        retained
            .tokens
            .expect("retained tokens should exist")
            .refresh_token,
        refresh_token
    );
...
#[tokio::test]
#[serial(codex_auth_env)]
async fn unauthorized_recovery_reloads_rotated_token_from_pool_database() {
    let codex_home = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    let server = MockServer::start().await;
    ...
    .await;
    let stale_access_token = manager
        .auth_cached()
    ...
    .await
    .expect("pool should reopen");
    let mut pool_auth = pool
        .read_account_tokens(WORKSPACE_ID_ALLOWED)
        .await
        .expect("pool auth should load")
        .expect("pool auth should exist");
    let fresh_access_token =
        fake_access_token(WORKSPACE_ID_ALLOWED, Utc::now().timestamp() + 7_200);
    pool_auth
        .tokens
        .as_mut()
        .expect("tokens should exist")
        .access_token = fresh_access_token.clone();
    pool.write_account_tokens(WORKSPACE_ID_ALLOWED, &pool_auth)
        .await
        .expect("rotated pool token should save");

    let mut recovery = manager.unauthorized_recovery();
    let result = recovery
        .next()
        .await
        .expect("401 recovery should reload the pool authority");

    assert_eq!(result.auth_state_changed(), Some(true));
    assert_eq!(recovery.step_name(), "refresh_token");
    let cached_access_token = manager
        .auth_cached()
        .expect("auth should remain cached")
        .get_current_token_data()
        .expect("token data should exist")
        .access_token;
    assert_ne!(cached_access_token, stale_access_token);
    assert_eq!(cached_access_token, fresh_access_token);
    assert!(
        !get_auth_file(codex_home.path()).exists(),
        "pool recovery must not recreate or depend on auth.json"
    );
    server.verify().await;
}
