...
/// stored for `pool_account_id`. The directory is nested under
...
pub fn account_pool_secret_dir(codex_home: &Path, pool_account_id: &str) -> PathBuf {
    pool_root_dir(codex_home)
        .join(SECRET_ROOT_DIR)
        .join(hash_fragment(pool_account_id))
}
...
impl ChatgptAccountPool {
    ...
    async fn register_account(
        ...
        let mut pool_auth = auth.clone();
        pool_auth.pool_account_id = Some(metadata.account_id.clone());
        save_auth(
    ...
    pub async fn mark_current_account_rate_limited(
        ...
        if let Some(snapshot) = snapshot {
            // Only extend the cooldown from snapshots that are authoritative for
            // account-level cooldown (limit_id="codex" or unset). Non-codex snapshots
            // (e.g. "premium") are stored for history but must not override the cooldown
            // clock — the Go poller only considers "codex" when resetting cooldown, so
            // using a "premium" window here would create a value the poller can't clear.
            if snapshot_contributes_account_cooldown(snapshot)
                && let Some(exhausted_reset_at) = exhausted_reset_at(snapshot)
            {
                cooldown_until = Some(
                    cooldown_until
    ...
    pub(crate) async fn record_fetched_rate_limits(
        ...
        for snapshot in snapshots {
            if snapshot_contributes_account_cooldown(snapshot)
                && let Some(reset_at) = exhausted_reset_at(snapshot)
            {
...
fn snapshot_contributes_account_cooldown(snapshot: &RateLimitSnapshot) -> bool {
    snapshot
        .limit_id
        .as_deref()
