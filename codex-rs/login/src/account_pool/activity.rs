...
impl ChatgptAccountPool {
    ...
    pub(crate) async fn record_account_activity_for_owner_at(
        ...
        // One process can have overlapping turns on different accounts after a
        // failover. The table's primary key includes account_id specifically so
        // those leases can coexist; clearing another account here would make an
        // older, still-streaming turn look idle to codex-accounts.
        ...
        .bind(now)
        .bind(now)
        .bind(now.saturating_add(ACCOUNT_ACTIVITY_TTL_SECONDS))
        .execute(&mut *tx)
        .await?;
        // Keep last_used_at current with each heartbeat so the capacity tiebreaker
        // reflects actual usage recency rather than just the turn-start activation time.
