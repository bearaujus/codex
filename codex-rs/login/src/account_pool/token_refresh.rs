...
impl ChatgptAccountPool {
    ...
    pub async fn persist_refreshed_account_auth(
        ...
        let now = now_ts();
        // Token bytes, validity acknowledgement, and its audit event describe
        // one refresh result. Commit them together so a failed event/retention
        // write cannot leave an unacknowledged rotated credential behind.
