...
Write-Phase "Preparing test run (profile=$CargoProfile, crate=$Crate)"
...
try {
    ...
    if ($UseCargoTest) {
        ...
        Invoke-Cargo -CargoArgs $Args -FailureMessage 'cargo test failed'
    }
    else {
        Write-Phase 'Selected cargo nextest execution path'
        Invoke-Cargo -CargoArgs $Args -FailureMessage 'cargo nextest failed'
    }
}
finally {
    Pop-Location
